...
void kdfi::collect_alloc()
{
  ...
  errs() << "normal_alloc : " << normal_alloc.size() << "\n";
  errs() << "priv_alloc   : " << priv_alloc.size() << "\n";
...
void kdfi::initialize_kdfi_struct()
{
    ...
    errs() << "Privileged Pointer: " << pptr.size() << "\n";
...
void kdfi::find_pstack_funcs() {
  ...
  //errs() << "find pstack func\n"; 
  //auto list = load_list(knob_pstack_list);
  //for (auto name : *list) {
  //  auto func = m->getFunction(name);
  //  if (func) {
  //    if (func->hasSection())
  //      if (func->getSection().contains(".init.text"))
  //        continue;
  ...
  errs() << "Priv stack  : " << pstack_func.size() << "\n";
}
