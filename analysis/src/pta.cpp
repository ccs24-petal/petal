...
void pta::collect_privilege_checks()
{
  ...
  for (auto pf : proc_funcs) {
    auto s = pf->pobj2inst;
    for (auto o : *pf->pobj) {
      auto sname = o->getStructName();
      if (sname.startswith("struct.anon.") || sname.startswith("union.anon"))
        continue;
      pobj.insert(o);
  ...
  pptr.insert(StructType::getTypeByName(*ctx, "struct.cgroup_root"));
